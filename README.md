...
現在は雛形の段階で、機能の実装はまだありません。自動テストは掲載写真の検査だけです。最初の山は「架空データの投入（seed）と、sandbox で動くバックエンド」で、本体のテストはそこで導入します。
...
図は SVG なので、ブラウザの拡大（⌘+）で文字がぼやけずに読めます。`docs/diagrams/architecture.py` から生成します（コマンドは下の表）。SVG は直接編集しません。
...
`public/pets/` にある6枚は、検索の動作確認に使うアンカー掲載の写真です。実在の動物ではなく、手元の Stable Diffusion 1.5 で作った架空の犬猫です。

生成する道具とプロンプトは、このリポジトリには置きません。別リポジトリ `pets-image`（未公開）が持ち、こちらは出来上がった画像だけを持ちます。素材づくりはポータルの機能ではないためです。`npm run test:pet-assets` は、置かれた画像が 512×512 かつ 250KB 以内の WebP で、6枚とも中身が違うことだけを確かめます。
