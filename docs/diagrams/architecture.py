...
FONT = "Hiragino Sans"
# 固定幅ノードなので、ラベルは 1 行 10 文字程度に改行し、横の間隔（nodesep）でぶつかりを避ける
GRAPH = {"fontname": FONT, "fontsize": "14", "pad": "0.5", "nodesep": "0.9", "ranksep": "0.75", "splines": "spline"}
# 既定（幅 1.4 / 高さ 1.9 インチ）はアイコンが大きすぎるので縮める
NODE = {"fontname": FONT, "fontsize": "12", "width": "0.9", "height": "1.6"}
...
with Diagram(
    ...
    graph_attr={**GRAPH, "ranksep": "0.9"}, node_attr=NODE, edge_attr=EDGE,
):
...
with Diagram(
    ...
    with Cluster("A. 開発・検証 — npx ampx sandbox（いつでも作って消せる）"):
        ...
        outputs = Document("amplify_outputs\n.json\n接続先が書かれる")
